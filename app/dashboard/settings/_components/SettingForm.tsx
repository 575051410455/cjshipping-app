...
import { CustomSpinner } from '@/components/ui/CustomeSpinner';
...
export const SettingsForm = ({ user }: { user: Session['user'] }) => {
  ...
  const title = user ? 'Profile Settings' : 'Edit Product'
      ...
      <Form {...form}>
        ...
        >
          <div className='space-y-4'>
            ...
            <NewPasswordConfirmField disabled={!user || isPending} form={form} isOauthAccount={user.isOauth} />
          ...
          <Button disabled={isPending} type='submit'>
            {isPending ? <CustomSpinner /> : 'Update account'}
          </Button>
        </form>
      </Form>
    </>
...
const NewPasswordConfirmField = ({
    ...
    const newPasswordConfirm = form.watch('password');
    return (
      <FormField
        control={form.control}
        ...
        render={({ field }) => (
          <FormItem
            className={
              isOauthAccount || disabled || !newPasswordConfirm
                ? 'cursor-not-allowed opacity-50 [&_*]:cursor-not-allowed'
                : ''
            }
          >
            ...
            <FormControl>
              <Input
                {...field}
                disabled={disabled || !newPasswordConfirm}
                onChange={(e) => {
                  const value = e.target.value;
                  field.onChange(value === '' ? undefined : value);
                }}
                placeholder='*****'
                type='password'
                value={!newPasswordConfirm ? '' : field.value}
              />
            </FormControl>
            <FormMessage />
          </FormItem>
